...
PlanetSelect.teams_configured = PlanetSelect.teams_configured or {}

PlanetSelect.start_on = function(planet, player)
  local team_name = "team_" .. planet
  local force = game.forces[team_name] or game.create_force(team_name)

  if not PlanetSelect.teams_configured[team_name] then
    dlog("Creating and configuring new team for planet: " .. team_name)
    force.set_spawn_position({0, 0}, game.surfaces[planet])
    PlanetSelect.teams_configured[team_name] = true
  end
